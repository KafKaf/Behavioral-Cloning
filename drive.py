...
class SimplePIController:
    def __init__(self, Kp, Ki):
        ...
        self.error = 0.
        self.integral = 0.
        self.deque = deque([], maxlen=moving_average_window_size)
...
def pre_process_image(image):
    # Crop image to remove noise Y1:Y2,X1:X2
    ...
    final_image = cv2.cvtColor(blurred_image, cv2.COLOR_RGB2YUV)
    return final_image
...
def telemetry(sid, data):
    if data:
        ...
        # predict steering angle based on the processed image
        steering_angle = float(model.predict(processed_image[None, :, :, :], batch_size=1))
        
        # smooth the steering angle based on past steering angles for a smoother driving experience
        smoothed_steering_angle = past_moving_average(controller.deque, steering_angle)

        throttle = controller.update(float(speed))
        
        print(smoothed_steering_angle, throttle)
